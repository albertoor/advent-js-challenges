...
criptomoneda en un día y debemos devolver la ganancia máxima que 
podríamos sacar si compramos y vendemos la inversión el mismo día.

La lista de precios es un array de números y representa el tiempo de 
izquierda a derecha. Por lo que ten en cuenta que no puedes comprar 
a un precio que esté a la derecha de la venta y no puedes vender a 
...
const pricesBtc = [87, 14, 25, 16, 30, 28, 6]
maxProfit(pricesBtc) // -> 16 (compra a 18, vende a 34)
...
function maxProfit(prices) {
  ...
  const buy = Math.min(...prices.splice(0, half));
  const sell = Math.max(...prices.splice(-half));
...
// const prices = [87, 14, 25, 16, 30, 28, 6];
...
// const pricesAda = [3, 3, 3, 3, 3];`
// console.log(maxProfit(pricesAda)); // -> -1 (no hay ganancia p
